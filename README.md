...
当前支持三国杀、狼人杀、国际象棋、中国象棋。所有游戏共用模型交流、即时反思、赛后复盘、经验归纳、并行调度与回放；经验按玩家和游戏类型隔离。
...
[观战界面与切换指南](docs/ARENA_UI.md) · [游戏与规则目录](docs/games/README.md) · [多游戏架构](docs/MULTIGAME_ARCHITECTURE.md) · [扩展游戏指南](docs/EXTENDING_GAMES.md) · [API](docs/API.md)

狼人杀使用明确的固定角色实验规则；中国象棋支持标准走法、将死/困毙与长将判负，复杂长捉竞赛裁定不在当前范围。国际象棋支持易位、吃过路兵、升变和重复/回合数和棋规则。详细边界以各游戏规则文档为准。下方展示与公开基线数据仍以三国杀为背景。
...
每位玩家独立配置模型 API，也可接入本地策略或外部 Agent。三国杀身份先随机分配、支持手动调整；Agent 在决策时自主发言或沉默，试探、协商与施压，本局聊天进入后续决策和复盘上下文。
...
打开[游戏大厅](http://localhost:3930)，选择游戏后点击 **「运行本地演示」**，即可观看策略 Agent 对战，无需 API Key。
...
模型接收自己的可见状态、合法动作、历史、聊天与个人经验，由规则引擎校验并执行选择。出牌与可选发言共用一次决策请求；弃牌阶段合并选牌，唯一合法行动通常自动执行，狼人杀讨论回合仍需 Agent 决策。模型调用失败或动作无效时，日志记录重试与本地兜底情况。
