...
<p align="center"><strong>在四款游戏中研究 Agent 的决策与经验复用</strong></p>
...
Strategy-RSI 用三国杀、狼人杀、国际象棋和中国象棋研究 Agent 如何积累和使用对局经验。你可以让不同模型对战，查看它们的行动、交流和复盘，再用导出的记录分析经验是否改善了后续决策。四款游戏共用模型接入和 RSI 服务，各自维护规则与玩家可见的信息。

从游戏大厅选择游戏，也可以按中英文名称搜索。返回大厅即可切换；每款游戏都会保留上次的对战、对局、回放位置和观战视角。
...
四款游戏都有独立的观战界面，支持 Agent 交流、即时／赛后 RSI，以及经验归纳和跨局复用。点击截图可查看原图。
...
<table>
  <tr>
    <td width="50%" valign="top">
      ...
      <p>玩家需要保留有用的手牌，根据行动和发言判断他人阵营。支持身份配置、基础武将与公开交流，适合观察模型如何判断盟友、分配卡牌资源和复用经验。双人局采用主公对反贼的简化规则。</p>
    ...
    <td width="50%" valign="top">
      ...
      <p>使用狼人、预言家、女巫、猎人与村民的固定角色配置。夜间行动后进入白天讨论和投票放逐，直到一方获胜。狼队可以夜间密谈；发言和投票记录可用于分析模型如何识别欺骗、建立信任和配合队友。</p>
  ...
  <tr>
    <td width="50%" valign="top">
      ...
      <p>支持标准走法，包括王车易位、吃过路兵和升变，按将死、提和及重复局面等规则结算。双方可随走子公开交流，复盘记录可用于分析局面判断、长期规划和经验复用。</p>
    ...
    <td width="50%" valign="top">
      ...
      <p>支持标准走法、将死、困毙与长将判负，双方可随走子公开交流。实验可观察模型如何保护将帅、配合子力；复杂长捉等竞赛裁定尚未实现，具体范围见规则说明。</p>
      <p><a href="docs/games/xiangqi.md">走法与实验室裁定 →</a></p>
    </td>
  </tr>
</table>

狼人杀和国际象棋可在创建对战时选择语言，游戏场地、规则说明及 Agent 决策／交流／RSI 提示词随之切换。共享大厅、玩家库和部分管理控件仍为中文。三国杀和中国象棋目前仅支持中文。
...
1. 在“玩家库”创建玩家，设置名字、模型连接与 RSI 模式。
2. 进入游戏并创建对战，选择语言、玩家、总局数、并行数与交流开关。
3. 查看实时行动、交流和回放，在经验面板中检查反思记录。后续同类游戏可以读取这些经验。
...
模型通过文本 JSON 读取棋盘或手牌。输入包含当前玩家可见的状态、合法动作、历史与交流，以及个人经验；模型返回行动 ID 和可选发言，由规则引擎校验后执行。网页截图仅用于观战。[输入格式与 API 示例 →](docs/API.md)
...
这里的 RSI 将文本反思保存为经验，再加入后续决策的上下文，不训练模型权重。评估时需分别比较空经验 Baseline、冻结经验和持续 RSI，并控制对手、座位、种子及调用失败情况。[实验设计与已有报告 →](exp/README.md)

<a id="架构与扩展"></a>

## 架构与扩展

游戏插件处理规则、可见信息、合法动作、胜负与状态恢复。模型调用、交流、RSI、经验、调度与存档由公共服务处理。添加游戏时，实现 `GamePlugin` 接口，再注册游戏元信息和前端场地即可接入这套服务。
...
四款游戏的报告按游戏分别保存。狼人杀、国际象棋和中国象棋各安排了 **168 局基线**，计入试跑和功能验证后，每款仍在 **200 局以内**。三国杀沿用此前的 **528 局实验**。基线均关闭 RSI，每次决策传入空经验；另有交流与 RSI 功能检查，但尚未测量 RSI 的学习收益。

| 游戏                              | 正式排程 | 完成 | 异常 | 含试跑与验证 / 上限 |
| --------------------------------- | -------- | ---- | ---- | ------------------- |
| [狼人杀](exp/werewolf/README.md)  | 168      | 156  | 12   | 176 / 200           |
| [国际象棋](exp/chess/README.md)   | 168      | 77   | 91   | 180 / 200           |
| [中国象棋](exp/xiangqi/README.md) | 168      | 73   | 95   | 175 / 200           |

<details>
<summary><strong>展开狼人杀：168 局结果与六张图表</strong></summary>

原始 24 局加上新增 144 局，结果为：规则胜负 156、规则和棋 0、上限平局 0、异常 12。
...
<table>
  <tr>
    <td width="50%" valign="top">
      <a href="exp/werewolf/assets/win-rate.svg"><img src="exp/werewolf/assets/win-rate.svg" alt="狼人杀模型表现及分组区间" width="100%" /></a>
    </td>
    <td width="50%" valign="top">
      <a href="exp/werewolf/assets/role-win-rate.svg"><img src="exp/werewolf/assets/role-win-rate.svg" alt="狼人杀角色表现" width="100%" /></a>
    </td>
  </tr>
</table>

使用 21 个角色种子，每个种子运行四种模型轮换和中英文两个版本。每模型同局占两席，统计时按角色种子分组，避免把相关席位视为独立样本。

[完整报告、RSI 检查与六组图表](exp/werewolf/README.md) · [results.json](exp/werewolf/results.json) · [plan.json](exp/werewolf/plan.json)

</details>

<details>
<summary><strong>展开国际象棋：168 局结果与六张图表</strong></summary>

原始 24 局加上新增 144 局，结果为：规则胜负 55、规则和棋 21、上限平局 1、异常 91。
...
<table>
  <tr>
    <td width="50%" valign="top">
      <a href="exp/chess/assets/win-rate.svg"><img src="exp/chess/assets/win-rate.svg" alt="国际象棋模型表现及分组区间" width="100%" /></a>
    </td>
    <td width="50%" valign="top">
      <a href="exp/chess/assets/head-to-head.svg"><img src="exp/chess/assets/head-to-head.svg" alt="国际象棋两两交锋" width="100%" /></a>
    </td>
  </tr>
</table>
...
[完整报告、RSI 检查与六组图表](exp/chess/README.md) · [results.json](exp/chess/results.json) · [plan.json](exp/chess/plan.json)

</details>

<details>
<summary><strong>展开中国象棋：168 局结果与六张图表</strong></summary>

原始 24 局加上新增 144 局，结果为：规则胜负 43、规则和棋 26、上限平局 4、异常 95。
...
<table>
  <tr>
    <td width="50%" valign="top">
      <a href="exp/xiangqi/assets/win-rate.svg"><img src="exp/xiangqi/assets/win-rate.svg" alt="中国象棋模型表现及分组区间" width="100%" /></a>
    </td>
    <td width="50%" valign="top">
      <a href="exp/xiangqi/assets/head-to-head.svg"><img src="exp/xiangqi/assets/head-to-head.svg" alt="中国象棋两两交锋" width="100%" /></a>
    </td>
  </tr>
</table>
...
[完整报告、RSI 检查与六组图表](exp/xiangqi/README.md) · [results.json](exp/xiangqi/results.json) · [plan.json](exp/xiangqi/plan.json)
...
- **2026.09.17** — 三款新游戏分别扩展至 168 局基线，按游戏独立保存报告、分组统计、历史数据与六组统一尺寸图表。
- **2026.09.16** — 新增三款游戏的 72 局模型基线、真实 RSI 功能验证、六组图表与复现脚本。
