...
[文档导航](README.md)。先阅读[多游戏插件架构](MULTIGAME_ARCHITECTURE.md)了解模块边界，再用本页查阅公共调度、事务、交流、经验和前端运行细节。三国杀专有实现另行标明，其他游戏的状态和规则由独立插件提供。
...
URL 与本地存储保留各游戏的比赛、对局、回放位置和视角；切换观察页面不会控制后台运行。返回上一场时清空经验面板的玩家与草稿，避免跨场误写。操作与代码入口见[界面指南](ARENA_UI.md)。
