...
国际象棋基线共安排 **168 局**：原始 24 局加上本次扩展的 144 局，最终 77 局完成、91 局异常。计入此前试跑和功能验证后，共运行 180 局，上限为 200 局。异常局保留在记录中，没有补跑替换。

## 实验设置与场数

6 组模型配对 × 2 种执色 × 2 种语言 × 7 次重复 = 168 局。
...
在已完成的对局中，DeepSeek 的点估计最高，为 68.9%，95% 分组区间为 57.4%–79.4%。失败局被排除后，样本可能存在选择偏差；仅凭此图不能确定模型排名的统计显著性。
...
规则胜负 55、规则和棋 21、行动上限平局 1、异常 91。异常局没有胜负结果，不计入模型败局。

### 5. Token 消耗

[![Token 消耗](assets/token-use.svg)](assets/token-use.svg)

正式基线记录 7348 次 HTTP 请求；输入 48,002,952 Token、输出 13,273,576 Token，77 次请求缺少 usage。
...
先按模型配对分层，再在每对模型内部重采样重复组。每对 7 组，同组的执色和语言不拆开。固定随机种子 20260917，进行 20,000 次 bootstrap；每次重新计算完成样本的指标，取 2.5% 与 97.5% 分位数。异常局随整组重采样，但不进入完成分母。
...
HTTP 账本比保存的决策尝试多 40 条，来自协议兼容重试或暂停时取消的请求等。一次决策可能发送多个请求，请求数与对局数分别统计。

另以 8,192 Token 输出额度运行了 4 局功能验证，其中 1 局通过全部检查，记录 1 次兜底行动和 6 条调用错误，归纳成功 8 / 8。验证的行动上限为 20，用于检查短局流程，不计入上面的模型表现。
...
审计复核了 165 份已保存上下文，检查可见信息、提示词语言和经验所属游戏，并验证存档恢复与 HTTP 回放。72 次决策读取了已有经验，其中 40 次读取了归纳经验。
...
离线审计从每局初始状态重放了 6802 个行动，检查 7308 份模型输入的可见信息和空经验，并核对最终状态。全部 168 局通过审计。
