/** Small, resumable baseline using the production rules and model protocol, without fallback play. */
...
const sources = [
  'scripts/run-multigame-baseline.ts',
...
const plan = {
  campaign: 'multigame-20260916',
  createdAt: new Date().toISOString(),
  models: expectedModels,
  modelLabels,
  baselineGames: 72,
  validationGames: 10,
  originalValidationGames: 10,
  pilotGames: 11,
  totalPlannedGames: 103,
  hardGameLimit: 200,
  sourceCommit: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(),
  sourceHashes: Object.fromEntries(sources.map((file) => [file, sha256(file)])),
  rules: Object.fromEntries(
    ['chess', 'xiangqi', 'werewolf'].map((g) => [g, GAME_CATALOG[g as GameType].rulesVersion]),
  ),
  settings,
  jobs,
};
...
assert.equal(new Set(jobs.map((j) => j.id)).size, 72);
for (const type of ['chess', 'xiangqi', 'werewolf'])
  assert.equal(jobs.filter((j) => j.gameType === type).length, 24);
for (const type of ['chess', 'xiangqi'])
  for (let model = 0; model < 4; model++)
    for (let seat = 0; seat < 2; seat++)
      assert.equal(jobs.filter((j) => j.gameType === type && j.models[seat] === model).length, 6);
...
const publicRoot = resolve('exp/multigame');
const planFile = join(publicRoot, 'plan.json');
if (existsSync(planFile)) {
  const saved = JSON.parse(readFileSync(planFile, 'utf8'));
  assert.deepEqual(
    saved.sourceHashes,
    plan.sourceHashes,
    'Experiment sources changed; do not mix runs.',
  );
  assert.deepEqual(saved.settings, plan.settings);
  assert.deepEqual(saved.models, plan.models);
...
const output = join(experimentRoot, 'baseline');
        ...
        for (
          ...
          try {
            ...
            if (/HTTP (401|403)/.test(lastError)) {
              atomicJSON(join(experimentRoot, 'baseline', 'STOP'), {
                at: new Date().toISOString(),
                reason: lastError,
              });
              stop();
